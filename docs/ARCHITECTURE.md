...
plaintext management application listener. Two loopback-only handlers emit
HTTP-to-HTTPS redirects, while `sslh` multiplexes plaintext and TLS connections
on public ports 8080 and 8081. The manager accepts JSON generated by a small
static HTML page and invokes only fixed control operations without constructing
shell commands from request values. The public sockets are restricted to
`eth0`; the firewall blocks both Web UIs on VPN interfaces.
